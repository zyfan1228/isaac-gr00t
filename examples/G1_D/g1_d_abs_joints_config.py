...
g1_d_abs_joints_config = {
    ...
    "action": ModalityConfig(
        delta_indices=list(range(0, 40)),  # predict 64 future steps
        modality_keys=[
            "left_arm",
            "right_arm",
            "left_gripper",
        ...
        action_configs=[
            # left_arm: ABSOLUTE = target position
            ActionConfig(
                rep=ActionRepresentation.ABSOLUTE, # absolute to current position
                type=ActionType.NON_EEF, # joint-space, not end-effector
                format=ActionFormat.DEFAULT,
            ),
            # right_arm: ABSOLUTE = target position
            ActionConfig(
                rep=ActionRepresentation.ABSOLUTE, # absolute to current position
                type=ActionType.NON_EEF, # joint-space, not end-effector
                format=ActionFormat.DEFAULT,
...
register_modality_config(g1_d_abs_joints_config, embodiment_tag=EmbodimentTag.G1_D_ARMS_JOINTS)