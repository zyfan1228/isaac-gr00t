...
class Gr00tN1d7Pipeline(ModelPipeline):
    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            ...
            if self.config.model.action_horizon != model.config.action_horizon:
                logging.info(
                    f"Overriding action_horizon: {model.config.action_horizon} -> {self.config.model.action_horizon}"
                )
                model.config.action_horizon = self.config.model.action_horizon
                model.action_head.action_horizon = self.config.model.action_horizon
    ...
    def _create_dataset(self, save_cfg_dir: Path):
        ...
        if self.config.training.start_from_checkpoint is not None:
            ...
            # from_pretrained ignores override kwargs; force-set critical dims
