...
class Gr00tN1d7ActionHead(nn.Module):
    ...
    def _init_actions(self, batch_size: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        return torch.randn(
    ...
    # lerobot rtc pipeline within get action
    @torch.no_grad()
    def get_action_with_rtc(
        ...
        device = vl_embeds.device
        actions = self._init_actions(batch_size, device, vl_embeds.dtype)

        inference_delay = options.get("inference_delay", 0) if options else 0
        action_horizon_input = (
            options.get("action_horizon", actions.shape[1]) if options else actions.shape[1]
        )
        # Truncate prev_chunk to action_horizon_input so the tail (zero-padded for
        # processor shape compat) is dropped. The model will internally zero-pad
        # back to full length in normalized space, which matches training.
        prev_chunk = None
        if "action" in action_input:
            prev_chunk = action_input["action"][:, :action_horizon_input]
        ...
        for t in range(self.num_inference_timesteps):
            ...
            def _make_step(t_idx):
                return lambda x: self._predict_velocity(
                    t_idx, x, vl_embeds, state_features, embodiment_id, backbone_output
                )

            corrected_velocity = rtc_processor.denoise_step(
                x_t=actions,
                prev_chunk_left_over=prev_chunk,
                inference_delay=inference_delay,
                time=t_cont,
                original_denoise_step_partial=_make_step(t),
            )
            actions = actions + dt * corrected_velocity
    ...
    def get_action_with_features(
        ...
        for t in range(self.num_inference_timesteps):
            pred_velocity = self._predict_velocity(
                t, actions, vl_embeds, state_features, embodiment_id, backbone_output
            )
...
class Gr00tN1d7(PreTrainedModel):
    ...
    def get_action_with_rtc(
        ...
        backbone_inputs, action_inputs = self.prepare_input(inputs)
        backbone_outputs = self.backbone(backbone_inputs)
        action_outputs = self.action_head.get_action_with_rtc(
            backbone_outputs, action_inputs, rtc_processor, options
        )
        return action_outputs
